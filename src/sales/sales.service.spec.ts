...
describe('SalesService', () => {
  ...
  describe('create', () => {
    it('should create order with items and calculate amounts', async () => {
      ...
      jest.spyOn(productsService, 'findSkuById').mockResolvedValue(mockSku as any);
    ...
    it('should throw when SKU not found', async () => {
      ...
      await expect(service.create(dto, 'user-1')).rejects.toThrow('SKU invalid not found');
    });

    it('should throw when product not found for SKU', async () => {
      jest.spyOn(productsService, 'findSkuById').mockResolvedValue({ id: 'sku-1', product: null } as any);
      ...
      await expect(service.create(dto, 'user-1')).rejects.toThrow('Product for SKU sku-1 not found');
    });
    ...
    it('should apply discount to line amount', async () => {
      const mockSku = {
        id: 'sku-1',
        skuCode: 'SKU001',
        product: { id: 'prod-1', name: 'Product', launchDate: null, lifecycleStage: null },
      };
      jest.spyOn(productsService, 'findSkuById').mockResolvedValue(mockSku as any);
  ...
  describe('submit', () => {
    it('should submit draft order for approval', async () => {
      ...
      const orderRepo = { findOne: jest.fn().mockResolvedValue(mockOrder), save: jest.fn((v) => Promise.resolve(v)) };
      Object.defineProperty(service, 'orderRepo', { value: orderRepo, writable: true });

      jest.spyOn(approvalService, 'submitForApproval').mockResolvedValue(undefined);

      const result = await service.submit('order-1', 'feishu-user-1', 'approval-code-1');
    ...
    it('should throw when order not found', async () => {
      ...
      await expect(service.submit('invalid', 'feishu-user-1', 'code')).rejects.toThrow('Order not found');
    });
    ...
    it('should throw when order is not draft', async () => {
      ...
      await expect(service.submit('order-1', 'feishu-user-1', 'code')).rejects.toThrow('Only draft order can be submitted');
    });
  });
  ...
  describe('batchPushJushuitan', () => {
    it('should push approved order to Jushuitan successfully', async () => {
      const mockOrder = {
        ...
        items: [
          { skuId: 'sku-1', skuName: 'Product A', skuCode: 'SKU001', jstSkuId: 'jst-001', qty: 1, unitPrice: 100 },
        ],
      ...
      Object.defineProperty(service, 'orderRepo', { value: orderRepo, writable: true });
      jest.spyOn(productsService, 'findSkuById').mockResolvedValue({ id: 'sku-1', skuCode: 'SKU001', jstSkuId: 'jst-001' } as any);
    ...
    it('should fail when order is not approved', async () => {
      ...
      expect(result.failed[0].reason).toBe('Only approved orders can be pushed');
    });
    ...
    it('should fail when items missing jstSkuId', async () => {
      const mockOrder = {
        ...
        status: SalesOrderStatus.APPROVED,
        signer: { jushuitanShopId: 'shop-1' },
        items: [{ skuId: 'sku-1', skuName: 'Missing SKU', skuCode: null, jstSkuId: null }],
        customer: { name: 'Customer' },
      };
      const orderRepo = { findOne: jest.fn().mockResolvedValue(mockOrder) };
      Object.defineProperty(service, 'orderRepo', { value: orderRepo, writable: true });
      jest.spyOn(productsService, 'findSkuById').mockResolvedValue({ id: 'sku-1', skuCode: null, jstSkuId: null } as any);
    ...
    it('should handle Jushuitan API failure', async () => {
      const mockOrder = {
        ...
        status: SalesOrderStatus.APPROVED,
        signer: { jushuitanShopId: 'shop-1' },
        items: [{ skuId: 'sku-1', skuName: 'Product', skuCode: 'SKU001', jstSkuId: 'jst-001' }],
        customer: { name: 'Customer' },
      };
      const orderRepo = { findOne: jest.fn().mockResolvedValue(mockOrder), save: jest.fn() };
      Object.defineProperty(service, 'orderRepo', { value: orderRepo, writable: true });
      jest.spyOn(jstService, 'createSalesOrder').mockResolvedValue({ code: 1, msg: 'API Error' } as any);
  ...
  describe('submitCollectionForApproval', () => {
    it('should submit collection for approved order', async () => {
      ...
      const orderRepo = { findOne: jest.fn().mockResolvedValue(mockOrder), save: jest.fn((v) => Promise.resolve(v)) };
      Object.defineProperty(service, 'orderRepo', { value: orderRepo, writable: true });
      jest.spyOn(approvalService, 'submitCollectionForApproval').mockResolvedValue(undefined);
      ...
      const result = await service.submitCollectionForApproval('order-1', dto, 'feishu-user-1', 'code');
    ...
    it('should throw when order status does not allow collection', async () => {
      ...
      await expect(service.submitCollectionForApproval('order-1', dto, 'user', 'code')).rejects.toThrow('订单状态不允许回款');
    });
    ...
    it('should throw when collection exceeds remaining amount', async () => {
      ...
      await expect(service.submitCollectionForApproval('order-1', dto, 'user', 'code')).rejects.toThrow('回款金额超过剩余应收款');
    });
    ...
    it('should throw when prepayment exceeds customer balance', async () => {
      ...
      await expect(service.submitCollectionForApproval('order-1', dto, 'user', 'code')).rejects.toThrow('客户预付款余额不足');
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should throw when order not found', async () => {
      ...
      await expect(service.findOne('invalid')).rejects.toThrow('Sales order not found');
    });
  });
});
