...
const ProductImage = ({ src }: { src?: string }) => {
  ...
  return (
    <div
      style={{ width: 40, height: 40, overflow: 'hidden', borderRadius: 10 }}
    >
      {status !== 'loaded' && (
        <div
          style={{
...
export default function ProductInventoryPage() {
  ...
  const handleSaveSafety = async (values: any) => {
    try {
      await axios.patch(
        `/stocks/${encodeURIComponent(safetySkuId)}/${encodeURIComponent(values.warehouseId)}/safety-stock`,
        {
  ...
  const openLifecycleModal = (record: SkuRow) => {
    ...
    lifecycleForm.setFieldsValue({
      ...
      launchDate: record.product?.launchDate
        ? dayjs(record.product.launchDate)
        : null,
    });
  ...
  const columns = [
      ...
      render: (_: any, record: SkuRow) => (
        <Space>
          ...
          <div style={{ overflow: 'hidden', maxWidth: 180 }}>
            ...
            >
              {record.skuName || record.jstSkuId || '-'}
            </div>
          </div>
        </Space>
      ),
    ...
    },
    {
      ...
      render: (_: any, record: SkuRow) => {
        const stage =
          record.product?.lifecycleStage ||
          record.product?.inferredLifecycleStage;
        if (!stage) return '-';
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (_: any, record: SkuRow) => {
        if (record.bomVersion)
          return <Tag color="green">{record.bomVersion}</Tag>;
        return <Tag>未配置</Tag>;
      },
    },
    {
    ...
    <div style={{ width: '100%' }}>
      ...
      >
        ...
        <Space>
          ...
          <Button onClick={() => exportProducts()}>导出 Excel</Button>
          <Button
            type="primary"
            icon={<PlusOutlined />}
          ...
          <Space direction="vertical" style={{ width: '100%' }} size="large">
            <Card
              ...
              extra={
                <Button
                  size="small"
                  onClick={() => openLifecycleModal(detailRecord)}
                >
                  编辑生命周期
                </Button>
              }
              ...
              <Space align="start">
                ...
                <div>
                  ...
                  <div style={{ marginTop: 4, color: '#A0A0A0', fontSize: 12 }}>
                    上市时间：
                    {detailRecord.product?.launchDate
                      ? dayjs(detailRecord.product.launchDate).format(
                          'YYYY-MM-DD',
                        )
                      : '未设置'}
                  </div>
                  <div style={{ marginTop: 4 }}>
                    {(() => {
                      const stage =
                        detailRecord.product?.lifecycleStage ||
                        detailRecord.product?.inferredLifecycleStage;
                      if (!stage)
                        return (
      ...
      >
        <Form form={createForm} layout="vertical" onFinish={handleCreateSubmit}>
          ...
          <div style={{ marginBottom: 8, fontWeight: 500, color: '#111111' }}>
            SKU 信息
          </div>
          <Form.Item
            label="SKU 编码"
